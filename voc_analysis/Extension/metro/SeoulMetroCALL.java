...
public class SeoulMetroCALL implements DbWatcherExtension {
	...
	private static String cate_query = "SELECT CATE_ID, UP_CATE_ID, CATE_NM FROM HT_IF_INF_CATE WHERE DIS_YN = 'Y'";
	private static ArrayList<HashMap<String, String>> categoryList = null;
	...
	public void getCategoyList() throws SQLException {
		PreparedStatement pstmtSelect = null;
		ResultSet rs = null;
		categoryList = new ArrayList<>();
		
		try {
			pstmtSelect = con.prepareStatement(cate_query);
			rs = pstmtSelect.executeQuery();
			while(rs.next()) {
				HashMap<String, String>tempMap = new HashMap<String, String>();
				tempMap.put("CATE_ID", rs.getString("CATE_ID").trim());
				tempMap.put("UP_CATE_ID", rs.getString("UP_CATE_ID").trim());
				tempMap.put("CATE_NM", rs.getString("CATE_NM").trim());
	...
	public static String searchCategory(ArrayList<HashMap<String, String>> categoryList, String categoryId, String findType){
		String returnStr = "";
		
		for(HashMap<String, String>selectCate :  categoryList){
			if(selectCate.get("CATE_ID").equals(categoryId)){
				if(findType.equals("UP_CATE_ID")){
					returnStr = selectCate.get("UP_CATE_ID");
					break;
				}
				else if(findType.equals("UP_CATE_NM")){
					String up_cate_id = selectCate.get("UP_CATE_ID");
					for(HashMap<String, String>upCate :  categoryList){
						if(upCate.get("CATE_ID").equals(up_cate_id)){
							returnStr = upCate.get("CATE_NM");
							break;
						}
					}
					break;
				}
				else if(findType.equals("CATE_NM")){
					returnStr = selectCate.get("CATE_NM");
					break;
				}
			}
		}
	...
	public void stop() throws SQLException {
		try{
			...
			categoryList.clear();
			if (con != null) {
				System.out.println("con !=null IN");
				con.close();
				watcher.releaseConnection(con);
				con = null;
			}
			if(disaAnalyzer != null) {
				System.out.println("disaAnalyzer !=null IN");
				disaAnalyzer.fine();
			}
			if(jianaCotton != null) {
				System.out.println("jianaCotton !=null IN");
				jianaCotton.fine();
			}
	...
	public FieldMapper getMapper(String fieldName, DbColumnValue[] columnValue) throws SQLException {
		...
		else if (fieldName.equals("CDVOCPART")) {
			return new CdVocPartMapper(columnValue);
		} 
		else if (fieldName.equals("VOCPART")) {
			return new VocPartMapper(columnValue);
		} 
		else if (fieldName.equals("VOCITEM")) {
			return new VocItemMapper(columnValue);
		} 
	...
	private static class CdVocPartMapper extends FieldMapper {
		private final int CDVOCITEM;

		public CdVocPartMapper(DbColumnValue[] columnValue) throws SQLException {
			super("CDVOCPART");
			CDVOCITEM = getIndexOf("CDVOCITEM", columnValue);
		}

		public String mapping(DbColumnValue[] value) {

			String cdvocitem = value[CDVOCITEM].getString();
			
			return searchCategory(categoryList,cdvocitem,"UP_CATE_ID");
		}
	}
	
	private static class VocPartMapper extends FieldMapper {
		private final int CDVOCITEM;

		public VocPartMapper(DbColumnValue[] columnValue) throws SQLException {
			super("VOCPART");
			CDVOCITEM = getIndexOf("CDVOCITEM", columnValue);
		}
		...
		public String mapping(DbColumnValue[] value) {
			...
			return searchCategory(categoryList,cdvocitem,"UP_CATE_NM");
		}
	}
	
	private static class VocItemMapper extends FieldMapper {
		private final int CDVOCITEM;

		public VocItemMapper(DbColumnValue[] columnValue) throws SQLException {
			super("VOCITEM");
			CDVOCITEM = getIndexOf("CDVOCITEM", columnValue);
		}
		...
		public String mapping(DbColumnValue[] value) {
			...
			return searchCategory(categoryList,cdvocitem,"CATE_NM");
		}
	}
