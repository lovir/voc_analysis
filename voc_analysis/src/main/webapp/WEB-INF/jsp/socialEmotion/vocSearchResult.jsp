...
<!-- 키워드랭킹분석 공통_하단 검색 결과(그룹+리스트) -->
	<c:choose>
		...
		<div class="win_head clear2">
			...
			<span class="win_date">
				<c:if test="${searchResultList.clickStack eq 'Y' }" >
				선택기간 : <c:out value="${searchResultList.printDateStr }" /> , "<span class="font_yb" id="share"><c:out value="${searchResultList.pnn }" /></span>" , 
				</c:if>
				검색결과수 : 
					...
					<c:forEach var="result" items="${searchResultList.listResult}" varStatus="status">
						<ul class="result_group">
							<li>
								...
								<dl>
									<dt> 
										<a href="javascript:;" onclick="detailView('<c:out value='${result.DQ_DOCID}' />'); return false;" class="modal_btn" >
