$(function () {
	...
	$('[name="title"]').live('click', function(){
		...
		$("#basic-modal-detail").load(getContextPath() + "/mainSearch/vocDetailView.do #detail",
				json,
			function(){
	...
	$('.result_doc').live('click', function(){
		...
		$("#basic-modal-alike").load(getContextPath() + "/mainSearch/vocAlikeView.do #alike",
				json,
			function(){
	...
	$('#btnSearch').on('click', function() {
		...
		if (validate()) {
			...
			$("#searchForm").attr('action', getContextPath() + "/mainSearch/vocSearch.do").submit();
		}
		return false;
	});
	...
	$(document).on('click', '.btn_xls', function(){
		$("#searchForm").attr('action', getContextPath()+"/mainSearch/vocExcelDownload.do").submit();
	});
...
function fnMove(){
	if($('#searchTerm').val().length<=0){
		...
		$("#headerForm").attr('action', getContextPath()+"/mainSearch/voeSearch.do");
		$("#headerForm").submit();
	}
}
...
function fnSearch() {
	...
	if (validate()) {
		$.ajax({
			type : "POST", 
			url : getContextPath() + "/mainSearch/vocList.do",
