...
// end of $(function(){ line 1
	...
	function displayRelation(div){
		$.ajax({//d3 차트 - 연관도 분석
			type : "post",
			url : getContextPath()+"/socialRelationAnalysis/compareRadarChart.do",
	...
	function displayLine(div){
		$.ajax({//하이차트 - 연관키워드 트렌드 분석
			type : "post",
			url : getContextPath()+"/socialRelationAnalysis/compareKeywordTrend.do",
			async : false,
			data : $("#searchForm").serialize(),
			success : function(data) {
				var returnData = $.parseJSON(data);
	...
	*/
	function fnsearchTotalList(div){
		$.ajax({
			type : "post",
			url : getContextPath()+"/socialRelationAnalysis/vocSearchResultCompare.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
	...
	$(document).on('click', '.btn_xls', function(){
		...
		$("#searchForm").attr('action', getContextPath() + "/socialRelationAnalysis/excelVocSearchResultCompare.do").submit();
	});

	//상세보기
	function detailView(id){
		$("#basic-modal-detail").load(getContextPath()+"/socialRelationAnalysis/detailViewCompare.do #detail",
		{"dq_docid" : id},
		function(){
	...
	$(document).on('click', '.result_doc', function(){
		...
		if( content != ""){
			$("#basic-modal-alike").load(getContextPath()+"/socialRelationAnalysis/getAlikeSearchCompare.do #alike",
