...
function dashInputVal(keyword, condition){
	...
	displayReport();
	$('input[name="clickPnn"]').val(keyword);
	fnDashSearchResult();
...
function fnDashSearchResult(){
	$.ajax({
		type : "post",
		url : getContextPath()+"/emotion/dashBoardSearchResult.do",
...
$(function(){
	...
	$(document).on('click', '.result_doc', function(){
		...
		if( content != ""){
			$("#basic-modal-alike").load(getContextPath() + "/emotion/getAlikeSearch.do #alike",
	...
	$(document).on('click', '.btn_xls', function(){
		...
		if(clickColumn == 'Y'){
			$("#searchForm").attr('action', getContextPath() + "/emotion/excelClickStackVocSearchResult.do").submit();
		}
		else{
			$("#searchForm").attr('action', getContextPath() + "/emotion/excelVocSearchResult.do").submit();
		}
	});
	...
	function displayReport(){
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/emotion/emotionTrendChart.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/emotion/emotionDistributionChart.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
				var returnData = $.parseJSON(data);
				if(returnData != undefined){
		...
		$.ajax({
			type : "POST",
			url : getContextPath() + "/emotion/repDeptList.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
				var returnData = $.parseJSON(data);
				// 처리주무부서 리스트 
	...
	*/
	...
	function fnSearchList(){
		$.ajax({
			type : "post",
			url : getContextPath()+"/emotion/vocSearchResultList.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
	...
	function fnSearchClickStackTotalList(){
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/emotion/vocClickStackSearchResult.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
				$('#vocSearchTeam').remove();
				$('#search_result').html(data);
	...
	function fnSearchClickStackList(){
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/emotion/vocClickStackSearchResultList.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
				$('#result_list').html(data);
				//$('#share').text($('#result_list input[name=share]').val()+'%');	//점유율 출력
	...
	function detailView(id){
		$("#basic-modal-detail").load(getContextPath() + "/emotion/detailView.do #detail",
		{"dq_docid" : id},
		function(){
