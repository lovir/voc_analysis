$(function () {
	...
	$('#btnAddEditKeyword').on('click', function(){
		...
		if($('#keyword').val().length>0){
			if($('#addKeyword').val().length>0){
				$('#thesaurusKeywordList p').each(function(){
							...
							$.ajax({
								...
								success : function(response) {
									if(response.chAddFlag == 1){
										$('#thesaurusKeywordList').append("<p style='color:red;' value='1'><span>" + $('#addKeyword').val() + "</span><a href='#' class='btn ssmall b_gray' id='del'>삭제</a></p>");
										...
										$('#thesaurusKeywordList').append("<p  value='0'><span>" + $('#addKeyword').val() + "</span><a href='#' class='btn ssmall b_gray' id='del'>삭제</a></p>");
	...
	$('#btnAdd').on('click', function(){
		...
		$('#thesaurusKeywordList p').each(function(){
			thesaurusKeywordList.push($(this).children('span').text());
		});
	...
	$('#del').live('click', function(){
		...
		$(this).parent().remove();
		if($('#thesaurusKeywordList p').length==0){
			//$('#btnAdd').attr("style", "color:#d2d2d2 !important;");
			$("#btnAdd").attr('disabled', 'disabled');
		}else{
			...
			success : function(response) {
				...
				if(data != null){
					for ( var i = 0, len = data.length; i < len; i++) {
						var result = data[i];
						$('#thesaurusKeywordList').append("<p><span>" + result + "</span><a href='#' class='btn ssmall b_gray' id='del'>삭제</a></p>");
					}
					//$('#btnAdd').attr("style", "color:#676767 !important;");
					$("#btnAdd").removeAttr("disabled");
				}else{
	...
	$('#btnSelectedDelete').on('click', function(){
		...
		if(selectedKeyword.length>0){
			...
			alert("삭제 할 키워드 좌측의 체크박스를 체크해 주세요.");
		}
		return false;
	});
	...
	// 페이지 번호 클릭
	$('#paging span').live('click', function(event){
		if($(event.target).attr('class') != 'on'){	//현재 선택한 페이지 아닌 경우 에만 작동
	...
	// 사전 업로드
	$('.btn_dic_add').click(function(){
		
		var dicType = $(this).attr('value');
		var vUwidth = "490";	//팝업창 가로크기
		var vUheight = "300";   //팝업창 세로크기
		var theURL = getContextPath() + "/management/uploadDictionaryInit.do?dicType="+dicType;
		var left = Math.ceil((window.screen.width - vUwidth) / 2 );
		var top = Math.ceil((window.screen.height - vUheight) / 2 );
		var win = window.open(theURL, "uploadDic", "top="+top+",left="+left+",toolbar=no,location=0,directories=no,status=yes,menubar=no,scrollbars=no,resizable=yes, width="+vUwidth+",height="+vUheight);
		win.focus();

	});
	
	// 사전 다운로드
	$('.btn_dic_down').click(function(){
		
		var dicType = $(this).attr('value');
		var vUwidth = "490";	//팝업창 가로크기
		var vUheight = "300";   //팝업창 세로크기
		var theURL = getContextPath() + "/management/downloadDictionaryInit.do?dicType="+dicType;
		var left = Math.ceil((window.screen.width - vUwidth) / 2 );
		var top = Math.ceil((window.screen.height - vUheight) / 2 );
		var win = window.open(theURL, "downloadDic", "top="+top+",left="+left+",toolbar=no,location=0,directories=no,status=yes,menubar=no,scrollbars=no,resizable=yes, width="+vUwidth+",height="+vUheight);
		win.focus();
