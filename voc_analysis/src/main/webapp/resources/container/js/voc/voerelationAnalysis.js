...
$(function(){
	...
	$('.btn_rank_search').click(function(){
		...
		$.ajax({//d3 차트 - 연관도 분석
			type : "post",
			url : getContextPath()+"/voerelation/radarChart.do",
		...
		$.ajax({//하이차트 - 연관키워드 트렌드 분석
			type : "post",
			url : getContextPath()+"/voerelation/keywordTrend.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/voerelation/vocSearchResult.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
	...
	$(document).on('click', '.result_doc', function(){
		...
		if( content != ""){
			$("#basic-modal-alike").load(getContextPath()+"/voerelation/getAlikeSearch.do #alike",
	...
	$(document).on('click', '.btn_xls', function(){
		$("#searchForm").attr('action', getContextPath()+"/voerelation/ExcelVocSearchResult.do").submit();
	});
	...
	function fnsearchList(){
		...
		$.ajax({
			type : "post",
			url : getContextPath()+"/voerelation/vocSearchResultList.do",
			data : $("#searchForm").serialize(),
			success : function(data) {
	...
	function detailView(id){
		$("#basic-modal-detail").load(getContextPath()+"/voerelation/detailView.do #detail",
		{"doc_id" : id},
		function(){
