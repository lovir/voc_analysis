$(function () {
	...
	$('[name="title"]').live('click', function(){
		...
		$("#basic-modal-detail").load(getContextPath() + "/mainSearch/userDetailView.do #detail",
				json,
			function(){
	...
	$('.result_doc').live('click', function(){
		...
		$("#basic-modal-alike").load(getContextPath() + "/mainSearch/userAlikeView.do #alike",
				json,
			function(){
	...
	$('#btnClear').on('click', function(){
		...
		$('#selectText').html('<font color="red">\'일별\'</font>은 시작일부터 일주일 설정만 가능합니다.');
		$('#userId').find('option:first').attr('selected',true);
	...
	$('#btnSearch').on('click', function() {
		...
		if (validate()) {
			...
			$("#searchForm").attr('action', getContextPath() + "/mainSearch/userSearch.do").submit();
		}
		return false;
	});
	...
	$(document).on('click', '.btn_xls', function(){
		$("#searchForm").attr('action', getContextPath()+"/mainSearch/userExcelDownload.do").submit();
	});
...
function fnSearch() {
	...
	if (validate()) {
		$.ajax({
			type : "POST", 
			url : getContextPath() + "/mainSearch/userList.do",
