...
$(function(){
	...
	$('.btn_rank_search').click(function(){
		...
		$.ajax({
			type : "post",     
			url : getContextPath() + "/voekeywordRanking/issueRanking.do",
...
/*	    $('#usesMediaTypeList > select').find('option:first').attr('selected',true);
*/	    $('#userLevelTypeList > select').find('option:first').attr('selected',true);
		$('#socialTypeList > select').find('option:first').attr('selected',true);

		$('#week_select').datepicker('setDate', new Date("2016/06/30"));
		$('#week_select').change();
	});
