$(function () {
	...
	$('[name="title"]').live('click', function(){
		...
		console.log("흐름");
		$("#basic-modal-detail").load(getContextPath() + "/voetrend/trendAnalysisDetailView.do #detail",
				json,
			function(){
				console.log("흐름1");
	...
	$('.result_doc').live('click', function(){
		...
		$("#basic-modal-alike").load(getContextPath() + "/voetrend/trendAnalysisAlikeView.do #alike",
				json,
			function(){
	...
	$('#btnClear').on('click', function(){
		...
		$('#socialType').find('option:first').attr('selected',true);
	...
	$('#btnSearch').on('click', function() {
		...
		if (validate()) {
			...
			$("#searchForm").attr('action', getContextPath() + "/voetrend/trendAnalysisSearch.do").submit();
		}
		return false;
	});
	...
	$(document).on('click', '.btn_xls', function(){
		$("#searchForm").attr('action', getContextPath()+"/voetrend/trendAnalysisExcelDownload.do").submit();
	});
...
function fnSearch(listFlg) {
	var url = "/voetrend/trendAnalysisList.do";
	if(listFlg){
		url = "/voetrend/trendAnalysisListAll.do";
	}
	if (validate()) {
		$.ajax({
			type : "POST", 
