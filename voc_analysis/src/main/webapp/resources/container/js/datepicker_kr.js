$(function() {
	...
	if(($('#startDate').length && $('#endDate').length) && ($('#startDate').val().length<=0 && $('#endDate').val().length<=0)){
		/*var sdate = new Date("2017/01/15");
		var edate = new Date("2017/01/21");*/
	...
	$('#calendar_select').change(function() {
		...
		if("DAY" == check){
			...
			$('#selectText').html('<font color="red">\'분기별\'</font>은 시작일부터 5년 설정만 가능합니다.');
			sdate.setMonth(edate.getMonth()-60);
		}else if("HALF" == check){
			$('#selectText').html('<font color="red">\'반기별\'</font>은 시작일부터 5년 설정만 가능합니다.');
			sdate.setMonth(edate.getMonth()-60);
		}else if("YEAR" == check){
			$('#selectText').html('<font color="red">\'년별\'</font>은 시작일부터 5년 설정만 가능합니다.');
			sdate.setMonth(edate.getMonth()-60);
		}
	...
	/*var sdateA = new Date("2017/01/15");
	var edateA = new Date("2017/01/21");*/
	...
	/*var sdateB = new Date("2017/01/15");
	var edateB = new Date("2017/01/21");*/
	var checkB = $('#calendar_selectB option:selected').val();
	
	if("DAY" == checkB){
		sdateB.setDate(edateB.getDate()-6);
	}else if("WEEK" == checkB){
		sdateB.setMonth(edateB.getMonth()-2);
	}else if("MONTH" == checkB){
		sdateB.setMonth(edateB.getMonth()-12);
	}else if("QUARTER" == checkB){
		sdateB.setMonth(edateB.getMonth()-24);
	}else if("HALF" == checkB){
		sdateB.setMonth(edateB.getMonth()-48);
	}else if("YEAR" == checkB){
		sdateB.setMonth(edateB.getMonth()-96);
	}
	
	$('#startDateB').datepicker('setDate', sdateB);
	$('#endDateB').datepicker('setDate', edateB);
